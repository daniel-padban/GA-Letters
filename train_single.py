...
if __name__ == '__main__':
    ...
    for dataset_n in range(sets_start,sets_end+1):
        ...
        import torch
        import wandb
        from experiment.trainer_def import CNNTrainer
        from experiment.init_weights import init_model_w
        ...
        #init wandb run
        model_name = f"Run-Demo-{train_subset_size}-S{seed}"
        run = wandb.init(name = model_name, config=config_dict,job_type='experimental run',group='demo')
        ...
        run.finish(0)
        dummy_input:torch.Tensor = train_dataset.__getitem__(0)[0]
        torch.onnx.export(trainer.model,dummy_input.unsqueeze(0).to(device),f'demo/demo_models/{model_name}.onnx')
